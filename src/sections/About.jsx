...
export function About() {
  ...
  return (
    <Section id="about" eyebrow={labels.about.eyebrow} title={labels.about.title}>
      <div className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr]">
        <GlassPanel className="rounded-[18px] p-7 shadow-interface">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="grid h-16 w-16 place-items-center rounded-[16px] border border-cyan/25 bg-cyan/10 text-cyan shadow-glow">
              <Fingerprint className="h-8 w-8" aria-hidden="true" />
            </div>
          </div>
          <div className="mt-8 rounded-[16px] border border-slate-200/80 bg-slate-50/85 p-6 dark:border-white/10 dark:bg-slate-950/70">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {profileFacts.map((fact) => (
            <GlassPanel key={fact.label} className="rounded-[16px] p-5 transition duration-300 hover:border-cyan/30 hover:bg-cyan/10 dark:hover:border-cyan/30 dark:hover:bg-cyan/10">
