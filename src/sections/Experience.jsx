...
export function Experience() {
  ...
  return (
    <Section id="activity" eyebrow={labels.activity.eyebrow} title={labels.activity.title}>
      <GlassPanel className="rounded-[18px] p-6 shadow-interface">
        <div className="relative">
          ...
          <div className="space-y-4">
            {timeline.map((item) => (
              <div key={`${item.period}-${item.title}`} className="relative grid gap-4 rounded-[16px] border border-slate-200/60 bg-slate-50/85 p-5 transition hover:border-cyan/30 dark:border-white/10 dark:bg-slate-950/70 sm:grid-cols-[10rem_1fr] sm:pl-12">
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400">
                    {item.tags?.map((tag) => (
                      <span key={tag} className="rounded-[9px] border border-slate-200/80 bg-slate-100/80 px-2 py-1 dark:border-white/10 dark:bg-slate-800/70">
                        {tag}
                      </span>
                    ))}
                  </div>
